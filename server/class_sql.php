...
class Sql
{
    ...
    $this->username = "c1320252_rojas";
    $this->password = "21KOzifuma";
    $this->dbname = "c1320252_rojas";
   ...
   /* $this->servername = "localhost";
    ...
    public function ShowFullTables($base){
        $this->connect();
        $sql = "SHOW FULL TABLES FROM $base";
      //  echo $sql;
        $result = $this->connection->query($sql);
        while($row = $result->fetch_assoc()) {
                    $this->columns[] = $row["Tables_in_".$base];
    ...
    public function selectTablaLike($tabla,$filtro,$filtro_por,$orden,$limit){
        $this->connect();
            if($filtro==="no"){
                $sql = "Select * FROM $tabla";
            }else{
                ...
                foreach($filtro as $dato=>$filtrar){
                        if ($dato === $this->endKey($filtro)) {
                            $sql .= "$dato LIKE '%$filtrar%'";
                        }else{
                            $sql .= "$dato LIKE '%$filtrar%' AND ";
                        }
                }
            }
        $sql .= " ORDER BY $filtro_por $orden ";
    ...
    public function selectBetween($tabla,$filtro,$orden,$limit,$que,$m){
        ...
        $this->connect();
            if($filtro==="no"){
                $sql = "Select * FROM $tabla WHERE ($que BETWEEN '$m' AND '$this->hoy')";
            }else{
                $sql = "Select * FROM $tabla WHERE ($que BETWEEN '$m' AND '$this->hoy') AND ";
    ...
    public function selectBetweenDos($tabla,$filtro,$orden,$limit,$que,$desde,$hasta){
        
                //echo "mes: $mes";
        $this->connect();
            if($filtro==="no"){
                $sql = "Select * FROM $tabla WHERE ($que BETWEEN '$desde' AND '$hasta')";
            }else{
                $sql = "Select * FROM $tabla WHERE ($que BETWEEN '$desde' AND '$hasta') AND ";
